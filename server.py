...
def recv(x,y):
    ...
    if bytes("/join","utf-8") in msg:                   #Join channels
            
            kanava = msg.decode("utf-8").split()[1]
            print(kanava)
            for i in channels:
                if channels[i] == kanava:
                    break
                else:
                    channels[i] == kanava
                    print("New channel",channels[i])
                    break
                    
        
        elif bytes("/pm","utf-8") in msg:                     #private messages        
...
def start():
    while True:
        ...
        channels = {"/1"}
        Thread(target=recv, args=(servu,address)).start()
